...
namespace ParamerticModuleModelling
{
    ...
    public class DataToSqlite_Stair : IExternalCommand
    {
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            List<string> Obj_List = new List<string>{ "三角钢板", "封边钢板", "承接钢板" ,"钢梁中心线", "构造钢筋","锯齿钢梁","平台悬挑","平台钢板","底衬控制线"};
                        ...
                        foreach (GeometryObject obj in geometry)
                        {
                            ...
                            if (gstyleName == "构造钢筋")
                                ...
                                if (gstyleName == "承接钢板")
                                {
                                    ...
                                    if (solid.Faces.Size == 8)
                                    {
                                        Extract_foldplate_Gemo(conn, solid, id, name, syname, gStyleId, gstyleName);
                                        continue;
                                    }
                                    else
                                    {
        ...
        private void Extract_foldplate_Gemo(SQLiteConnection conn, Solid solid, string id, string name, string syname, string gStyleId, string gstyleName)
        {
            ...
            foreach (Face f in f_list)
            {
                ...
                foreach (EdgeArray eloop in f.EdgeLoops)
                {
                    ...
                    List<XYZ> p_face_dics_CW = point_sort_CW(p_face);

                    List<string> p_face_dics = ConvertXYZ2String(p_face_dics_CW);
        ...
        private List<string> CalSrfThk_with_Srf(Face f1,FaceArray fArray,double thk0)//out double thk, out List<string>point_mid)
        {
            ...
            foreach (Face f2 in fArray)
            {
                ...
                if (Math.Abs(f2.Area - f1.Area)>1e-3)
                {
